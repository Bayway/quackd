...
| `quackd/sim3d/` | The physics world: the cartoon's arena, seeds, deadman, kick cone and scoop, in MuJoCo. `world.py` steps a `Body`, and two exist, a kinematic puppet that needs no download and upstream's own Microduck model walking on upstream's own `alpha_walking.onnx` at 50 Hz. `assets.py` fetches the model and the policies at a pinned commit into `~/.quackd/cache` and checks every file against a recorded sha256; `upstream_api.py` is the only file allowed to spell a `microduck_rl` name ([ADR-0030](adr/0030-mujoco-physics-backend.md)). |
...
| `web/` | The same loop in a browser, and the only quackd code that is not Python: MuJoCo compiled to WebAssembly, the same policy in onnxruntime-web, the same verbs, allowlist and budget, with the model and the policy fetched from the same pinned upstreams. It shares no code with the package, so it is kept in step by hand; `tests/test_web.py` holds the parts that can be checked from Python — the two upstream pins, the gait numbers, the ids the script looks up — and prints what to paste when they drift ([`web/README.md`](../web/README.md), [ADR-0030](adr/0030-mujoco-physics-backend.md)). |
