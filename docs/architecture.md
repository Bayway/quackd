...
| `web/` | The same loop in a browser, and the only quackd code that is not Python: MuJoCo compiled to WebAssembly, the same policy in onnxruntime-web, the same verbs, allowlist and budget, with the model and the policy fetched from the same pinned upstreams. It shares no code with the package and is kept in step by hand, which is the standing cost of it existing ([`web/README.md`](../web/README.md), [ADR-0030](adr/0030-mujoco-physics-backend.md)). |
