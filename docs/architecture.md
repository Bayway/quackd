...
| `bridge/toddlerbot/` | quackd's own ToddlerBot daemon: the fifty hertz loop upstream has no daemon for, plus the seven things it does not do at all - signal handlers that settle before exit, a hard-exit timer around a `close()` that holds the GIL, a safe-pose slew because no reset exists, an all-zeros detector, a joint clamp, a rate limit and a construction watchdog ([ADR-0028](adr/0028-toddlerbot.md)). |
