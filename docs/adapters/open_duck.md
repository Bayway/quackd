...
- The deadman is **quackd's own, running on the robot**, and it is evaluated inside the call
  the control loop makes every tick rather than by a timer. No command for 300 ms and the
  three velocities go to zero, even if the server thread is starved, wedged or dead.
...
- Head control is **off unless you ask**, then clamped to 80 percent of the runtime's range
  and rate limited. quackd never presses upstream's head-control mode button.
...
Start it first and point the bridge at it with `--camera-url`. **Without a URL the bridge
advertises no camera even when `duck_config.json` says the duck has one**, and `observe`,
...
|---|---|
| `PAD_SUBSTITUTION` | the daemon refuses to serve if upstream never constructed its controller, so a rename is a loud failure rather than a duck moving for invisible reasons |
