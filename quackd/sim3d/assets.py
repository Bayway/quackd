...
`QUACKD_CACHE_DIR` moves the cache. Nothing here runs in tests: they use the puppet body.
"""
...
log = logging.getLogger("quackd.sim3d")
...
def cache_root() -> Path:
    return Path(os.environ.get(CACHE_ENV) or Path.home() / ".quackd" / "cache")
...
def ensure_microduck(*, offline: bool = False) -> MicroduckAssets:
    ...
    if override:
        ...
        if _mismatches(robot_dir):
            ...
            log.warning(
                "fetching the Microduck model (about 10 MB) from %s into %s. %s",
                up.REPO,
                robot_dir,
                up.MESH_LICENSE.name,
            )
            written = _extract_robot_dir(fetch(up.TARBALL), robot_dir)
            bad = _mismatches(robot_dir)
            if bad:
                raise AssetError(
                    f"the fetched model does not match the pin ({written} files written, "
                    f"{len(bad)} wrong: {', '.join(bad[:3])}...). Upstream may have moved the "
                    f"commit's archive; please report this"
                )
            (robot_dir / NOTICE_FILE).write_text(NOTICE, encoding="utf-8")
    if _policy_mismatches(policies_dir):
        ...
        for name in _POLICY_FILES:
            (policies_dir / name).write_bytes(fetch(up.policy_url(name)))
        bad = _policy_mismatches(policies_dir)
        if bad:
            raise AssetError(f"a fetched policy does not match its pin: {', '.join(bad)}")
        (policies_dir / NOTICE_FILE).write_text(NOTICE, encoding="utf-8")
